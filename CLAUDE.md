...
- `npm run lighthouse` — builds, then LHCI against budgets in `.lighthouserc.json`. The URL list is one page per route template, not every page: the 19 tag pages and 18 project pages share a template each, and `numberOfRuns` is 3, so auditing them all would triple the runtime for duplicate signal. Add a URL when a new template appears, not a new entry.
