...
Unit tests (`node:test`, colocated `*.test.ts` in `src/lib/`) cover the Sätteri plugins end-to-end through the configured processor, plus the pure logic in `tag-slugs.ts`, `site.mjs`, and `adventure/format.ts`. Verification in CI is linting, type-checking, the tests, and the build. The pa11y accessibility audit (`npm run pa11y`), Lighthouse (`npm run lighthouse`), and link-check (`npm run link-check`) are all local-only and user-run — pa11y is worth running before merging an accessibility-affecting change and link-check after touching links, but say when a change warrants one rather than running it unprompted.
...
- **satteri 0.9.5 leaks camelCase SVG presentation attributes — write them kebab-case.** In plugin-created hast nodes, `className`/`ariaHidden`/`viewBox` map to correct HTML, but SVG presentation attributes are missing from the name tables: camelCase `strokeLinecap`/`strokeLinejoin` leak unconverted on both the .md and .mdx paths, and `strokeWidth` leaks on the .mdx path (invalid HTML, browsers ignore it). Kebab-case spellings (`"stroke-width"`, `"stroke-linecap"`, `"stroke-linejoin"`) survive verbatim everywhere — the glyph builders use those. Candidate upstream report; if satteri fixes its tables the kebab-case keys keep working. Related hazard: satteri silently ignores unknown visitor keys, so a typo'd `element`/`blockquote` key means the plugin never runs and the build stays green — `satteri-plugins.test.ts` exists largely to turn that (and the other silent failure modes on this list) into red tests.
...
- No test framework beyond Node's built-in `node:test` runner. Don't add one (vitest, playwright, etc.); the guardrails here are `astro check`, prettier, eslint, `npm test`, and the build. Tests stay confined to pure logic and the markdown pipeline — no DOM or component testing.
