...
- **`rehypeHeadingIds` is pinned first in the rehype list.** Astro injects its own copy _after_ every custom plugin, so `rehype-heading-anchors` (which needs the ids to build its `#slug` links) would see id-less headings otherwise. Astro's later pass keeps existing ids but re-collects each heading's text for `render(entry).headings` — so anything appended to a heading must carry its accessible name as `aria-label`, never as visually-hidden text, or the text leaks into that metadata.
